...
function App() {
  ...
  const handleAIEnhance = async () => {
      ...
      const key = settings.customApiKey || process.env.API_KEY || '';
      if (!key) {
          alert("API Key Required for AI Enhancement");
  ...
  const handleGeminiAnalysis = async (targetImage?: ProcessedImage) => {
    ...
    const key = settings.customApiKey || process.env.API_KEY || '';
    if (!key) { alert("API Key Required"); setShowSettings(true); return; }
                         ...
                         {!isPendingAiEnhance && (
                             <div className="space-y-3 pt-2 border-t border-gray-200 dark:border-gray-700">
                                 <button 
                                    ...
                                    className={`w-full py-3 px-4 rounded-xl flex items-center justify-center gap-2 shadow-lg transition-all transform font-bold ${
                                        hasSaved 
